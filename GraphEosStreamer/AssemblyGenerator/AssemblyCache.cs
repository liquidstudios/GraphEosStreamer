    ...
    public static class AssemblyCache
    {
        ...
        public static void LoadAssemblies()
        {
            foreach (var directory in Directory.GetDirectories(RuntimeAssemblyGenerator.AssemblyPath))
            {
                ...
                {
                    var abiPath = abiPaths.SingleOrDefault(a => a == dllPath.Replace(".dll", ""));
                    if (abiPath != null)
                    {
                        var abi = JsonConvert.DeserializeObject<Abi>(File.ReadAllText(abiPath + ".abi.bin"));
                        ...
                        if (!ContractAssemblyCache.ContainsKey(SerializationHelper.ConvertNameToLong(contractName)))
                            //                        ContractAssemblyCache.Add(contractName, new AbiTypeCacheEntry(assemblyTypes));
                            //                    else
